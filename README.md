...
复用 Animius 的 13 个番剧数据源解析器，另新增 **HiAnime** 源（ani-cli 同源，hianime.at，含 360/720/1080p 多画质），见 `data/remote/parse/`。网页抓取类源站存活率波动很大，
部分源的播放页依赖浏览器内核执行 JS（原 App 用 WebView 解决），服务端对这类源会解析失败并提示换线路。
可选配置出站代理（设置页，如 `http://127.0.0.1:10800`），被墙 CDN 的源依赖它。用哪个源在网页上下拉切换即可。
